...
data = reader.read(
    "\\\\TOCZ-FS2\\510-TOCZ\\300 Departments\\300 Technical Services\\Dokumentace B\\NACTENO\\TMP082757587.TXT"
)
...
# ── Configurators for position 10 ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("POSITION 30 — CONFIGURATORS")
print("=" * 60)

for name, cfg in data.get("positions", {}).get("30", {}).items():
    print(f"\n  {name}")
    print(f"    ID      : {cfg.get('id')}")
    print(f"    Výr.obj.: {cfg.get('vyr_obj')}")
    print("    First 10 characteristics:")
